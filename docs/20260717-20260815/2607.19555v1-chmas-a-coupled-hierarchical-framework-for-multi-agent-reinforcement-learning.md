...
score: 6.0
evidence: 面向协作控制的多智能体强化学习
tldr: 多智能体强化学习面临全局协调与局部执行的时间尺度矛盾。本文提出耦合分层框架CHMAS，战略层集中生成引导动作，战术层分布式执行并反馈累积奖励，通过耦合系数λ实现双向信息流。采用异步更新协议缓解非平稳性，理论证明战略层以O(logK/√K)收敛。在觅食任务中验证了空间分区探索策略，两层稳定收敛。
source: arxiv
...
method: 提出耦合分层框架，战略层集中规划、战术层分布式执行，通过λ耦合与异步更新实现双向交互。
result: 在觅食任务中学会空间分区策略，两层均稳定收敛，战略层取得O(logK/√K)收敛率。
conclusion: 证明分层耦合与异步训练可兼顾全局最优与局部可行，为层次化MARL提供新范式。
---

## 摘要
多智能体强化学习（MARL）系统在平衡不同时间尺度上的全局协调与局部执行方面面临根本性挑战。本文提出了耦合分层多智能体系统（CHMAS），一种新颖的框架，将多智能体决策分解为集中式战略规划和分布式战术执行，并具有双向信息流。战略层整合所有智能体的状态以及一个独占的全局环境状态，每隔 T 个时间步生成引导动作，而战术智能体则执行由战略引导和局部邻域观测增强的分布式策略。与现有单向控制的层级方法不同，CHMAS 建立了一种反馈机制，其中累积的战术奖励通过耦合系数 λ 影响战略目标，确保战略计划始终基于战术可行性。为了解决分层学习中固有的非平稳性，我们提出了一种异步更新协议，其中战略参数每 N_f 个战术回合更新一次，允许战术策略在战略变化之间收敛到准平稳点。我们提出了一个捕获完整系统动态的通用双层公式，以及一个可处理的加性近似，使得严格分析成为可能。理论分析证明，在标准假设下，这种异步方案在 K 次战略更新后，战略层实现了 O(log K/√K) 的收敛。在多智能体觅食领域的实验验证表明，该方法能成功学习空间划分的探索策略，尽管存在分层耦合，两层仍能稳定收敛。
